...
const OptionWheel = ({
  ...
  const applyTarget = useCallback(
    (value: number, snap: boolean) => {
      ...
      targetRef.current = v;
      const idx =
        ((Math.round(v) % cfg.count) + cfg.count) % cfg.count;
  ...
  // External control: sync the wheel when the host drives the selection
  useEffect(() => {
    if (driveIndex == null) return;
    if (driveIndex !== selectedRef.current) {
      applyTarget(driveIndex, true);
    }
  }, [driveIndex, applyTarget]);
